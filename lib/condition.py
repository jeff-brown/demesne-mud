...
class Condition():
    ...
    def check_rage(self, player, condition):
        """ check rage """
        print("check_rage")
                    ...
                    if skill["active"]:
                        ...
                        try:
                            player["equipped"]["weapon"]["damage"] = (
                                player["equipped"]["weapon"]["o_dam"].copy())
                        except KeyError as e:
                            print("Probably equipped a new weapon during rage event: ", e)
                    else:
                        return
        else:
            for skill in player["skills"]:
                if skill["type"] == "rage":
                    if skill["active"]:
                        ...
                        print("activate rage")
                        skill["active"] = True
                        player["equipped"]["weapon"]["o_dam"] = (
                            player["equipped"]["weapon"]["damage"].copy())
                        player["equipped"]["weapon"]["damage"].append(skill["bonus"])
                        skill["num"] -= 1
        ...
        for condition in player["conditions"]:
            if condition["repeating"]:
                if time.time() - condition["start"] > condition["duration"]:
                    ...
                    conditions.append(condition)
            elif 'trigger' in condition.keys():
                callback = getattr(self, condition["trigger"])
                print(condition["start"])
                print(time.time())
                print(int(time.time() - condition["start"]), condition["duration"])
                if time.time() - condition["start"] < condition["duration"]:

...
from lib.barbarian import Barbarian
...
class Game():
    ...
    def _get_skills(self, player):
        """calculate stat with species bonuses"""
        pclass = self._classes[player["class"]]
        if pclass["type"] == "barbarian":
            return self._barbarian.barbarian.copy()
    ...
    def _process_new_player(self, uid, command):
        ...
        self._players[uid]["skills"] = self._get_skills(self._players[uid])
        ...
        self._players[uid]["base_strength"] = self._get_stat(uid, "strength")
        self._players[uid]["base_dexterity"] = self._get_stat(uid, "dexterity")
        self._players[uid]["base_constitution"] = self._get_stat(uid, "constitution")
        self._players[uid]["base_intelligence"] = self._get_stat(uid, "intelligence")
        self._players[uid]["base_wisdom"] = self._get_stat(uid, "wisdom")
        self._players[uid]["base_charisma"] = self._get_stat(uid, "charisma")
        ...
        self._players[uid]["attack_bonus"] = 0
        ...
        self._players[uid]["base_armor_class"] = self._armor_class(uid)
    ...
    def _process_rest_command(self, uid, params):
        """
        output current level and experience
        """
        player = self._players[uid]
        condition = self._condition.get_status(player)
        print(condition)

        if condition != "Healthy":
            self._mud.send_message(uid, "You must be healthy to rest.")
            return
        ...
        if monsters_here:
            self._mud.send_message(uid, "It's not safe to rest here.")
            return

        if player["room"][0] == 1:
            self._mud.send_message(uid, "You're not allowed to rest here.")
            return

        player["conditions"].append(
            self._condition.set_condition("rest"))
        self._mud.send_message(uid, "You feel better after a short rest.")
    ...
    def _process_act_command(self, uid, params):
        ...
        if not params:
            self._mud.send_message(
                uid, "You need to specify what you want to activate.")
            return
        ...
        if has_skill:
            if not skill["active"]:
                if skill["num"] > 0:
                    player["conditions"].append(
                        self._condition.set_condition(skill["type"]))
                    self._mud.send_message(uid, skill["p_msg"])
                else:
                    self._mud.send_message(uid, skill["r_msg"])
            else:
                self._mud.send_message(uid, "That skill is already active.")
        else:
            self._mud.send_message(uid, "You don't seem to know how to do that.")
    ...
    def _process_go_command(self, uid, command, params):
        ...
        if 'resting' in self._condition.get_status(self._players[uid]).lower():
            # if time.time() - self._players[uid]["fatigue"] < self._tick:
            self._mud.send_message(
                uid, (
                    "Sorry, you can't move while you're resting."
                )
            )
            return
    ...
    def _process_attack(self, uid, target, spell=None):
        ...
        if 'resting' in self._condition.get_status(self._players[uid]).lower():
            ...
            self._mud.send_message(
                uid, (
                    "Sorry, you can't attack while you're resting."
                )
            )
            return
    ...
    def _process_melee_damage(self, uid, target):
        ...
        # TODO: Modify this to take level delta into account
        xp_incr = int(self._cr[target["cr"]] / target["max_hp"])
...
#     def _process_attack(self, uid, target, attack_type="melee", spell=None):
    ...
    def check_for_new_commands(self):
        ...
        for uid, command, params in self._mud.get_commands():
            ...
            elif command in ["act", "activate"]:

                self._process_act_command(uid, params)

            elif command in ["rest", "r"]:

                self._process_rest_command(uid, params)
